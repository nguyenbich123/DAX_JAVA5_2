				...
				<div class="col-lg-12">
					...
					<div class="card">
						<div class="card-body">
							...
							<!-- Horizontal Form -->
							<form:form action="/admin/ctsp/index" modelAttribute="ctsp"
								enctype="multipart/form-data">

								<form:hidden path="maCTSP" />
								<div class="row mb-3">
									<label for="inputPassword3" class="col-sm-3 col-form-label">Màu
										Sắc</label>
									<div class="col-sm-9">
										<form:select path="maMau" class="form-select">
											<c:if test="${!not empty ctsp.maMau }">
												<form:option value="">--Chọn Màu Sắc--</form:option>
												<form:options items="${list_m}" />
											</c:if>
											<c:if test="${not empty ctsp.maMau}">
												<form:option value="${ctsp.maMau.maMau}">${ctsp.maMau.mauSac}</form:option>
												<form:options items="${list_m}" />
											</c:if>
										</form:select>
								...
								<div class="row mb-3">
									<label for="inputPassword3" class="col-sm-3 col-form-label">Dung
										Lượng</label>
									<div class="col-sm-9 ">
										<form:select path="maDL" class="form-select">
											<c:if test="${!not empty ctsp.maDL }">
												<form:option value="">--Chọn Dung Lượng--</form:option>
												<form:options items="${list_dl}" />
											</c:if>
											<c:if test="${not empty ctsp.maDL}">
												<form:option value="${ctsp.maDL.maDL}">${ctsp.maDL.dungLuong}</form:option>
												<form:options items="${list_dl}" />
											</c:if>
										</form:select>
								...
								<div class="row mb-3">
									...
									<div class="col-sm-9 ">
										<form:select path="maRam" class="form-select">
											<c:if test="${!not empty ctsp.maRam }">
												<form:option value="">--Chọn Ram--</form:option>
												<form:options items="${list_r}" />
											</c:if>
											<c:if test="${not empty ctsp.maRam}">
												<form:option value="${ctsp.maRam.maRam}">${ctsp.maRam.ram}</form:option>
												<form:options items="${list_r}" />
											</c:if>
										</form:select>
								...
								<div class="text-center">
									<button formaction="/admin/ctsp/update?maSP=${maSP}"
